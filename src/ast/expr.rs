...
// /// 将 Vec<GenericAtom<HeadOrEq<ResolvedCall>, ResolvedVar>> 转换为 Vec<GenericExpr<String, String>>
// pub(crate) fn atoms_to_exprs(
//     atoms: Vec<GenericAtom<HeadOrEq<ResolvedCall>, ResolvedVar>>,
// ) -> Vec<GenericExpr<String, String>> {
//     atoms.into_iter().map(atom_to_expr).collect()
// }

// /// 将单个 GenericAtom<HeadOrEq<ResolvedCall>, ResolvedVar> 转换为 GenericExpr<String, String>
// fn atom_to_expr(
//     atom: GenericAtom<HeadOrEq<ResolvedCall>, ResolvedVar>,
// ) -> GenericExpr<String, String> {
//     match atom.head {
//         HeadOrEq::Eq => {
//             // 等式原子转换为相等表达式
//             assert_eq!(atom.args.len(), 2);
//             let left = atom_term_to_expr(&atom.args[0]);
//             let right = atom_term_to_expr(&atom.args[1]);
//             GenericExpr::Call(atom.span, "=".to_string(), vec![left, right])
//         }
//         HeadOrEq::Head(call) => {
//             // 函数调用原子转换为函数调用表达式
//             let args: Vec<GenericExpr<String, String>> = atom
//                 .args
//                 .into_iter()
//                 .map(|arg| atom_term_to_expr(&arg))
//                 .collect();

//             let func_name = match call {
//                 ResolvedCall::Func(func_type) => func_type.name,
//                 ResolvedCall::Primitive(prim) => prim.primitive.0.name().to_string(),
//             };

//             GenericExpr::Call(atom.span, func_name, args)
//         }
//     }
// }

// /// 将 GenericAtomTerm<ResolvedVar> 转换为 GenericExpr<String, String>
// pub fn atom_term_to_expr(atom_term: &GenericAtomTerm<ResolvedVar>) -> GenericExpr<String, String> {
//     match atom_term {
//         GenericAtomTerm::Var(span, var) => GenericExpr::Var(span.clone(), var.name.clone()),
//         GenericAtomTerm::Literal(span, lit) => GenericExpr::Lit(span.clone(), lit.clone()),
//         GenericAtomTerm::Global(span, var) => GenericExpr::Var(span.clone(), var.name.clone()),
//     }
// }
