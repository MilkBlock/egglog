...
fn add_math_rule(
    ...
    rust_rule(egraph, rule_name, ruleset, vars, facts, action).unwrap();
}
...
"#,
    ...
    add_math_rule(
        ...
        move |ctx, _| {
            ...
            let one_for_mul = m_const(ctx, 1);
            let _ = m_mul(ctx, x, one_for_mul);
...
#[allow(dead_code)]
pub fn assert_mul_one_rewrites(input: &mut MathMicroBenchInput) {
    let math = input.egraph.get_sort_by_name("Math").unwrap().clone();
    let x_name = input.egraph.base_to_value::<S>("x".to_owned().into());
    let x = input.egraph.lookup_function("MVar", &[x_name]).unwrap();
    let one_lit = input.egraph.base_to_value::<i64>(1);
    let one = input.egraph.lookup_function("MConst", &[one_lit]).unwrap();
    let mul = input.egraph.lookup_function("MMul", &[x, one]).unwrap();

    let x_canon = input.egraph.get_canonical_value(x, &math);
    let mul_canon = input.egraph.get_canonical_value(mul, &math);
    assert_eq!(
        mul_canon, x_canon,
        "mul_one rewrite should canonicalize to x"
    );
}
